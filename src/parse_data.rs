...
fn parse_regular(raw: &str) -> (Vec<Num>, Vec<Num>) {
    ...
    (parse_animation(&raw[..sep]), parse_animation(&raw[sep+1..]))
}
...
lazy_static::lazy_static! {
    pub static ref ZOMBIE_DB: HashMap<ZombieType, ZombieData> = {
        let file_content = include_bytes!("../assets/data.csv");
        let mut csv_reader = csv::Reader::from_reader(&file_content[..]);
        csv_reader.deserialize::<RawZombieData>()
            .map(Result::unwrap)
