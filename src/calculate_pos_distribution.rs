...
fn calculate_animation_impl(data: &ZombieData, ice_time: i64, time: i64, animation: Option<&Vec<Num>>) -> PosDistribution {
...
fn calculate_animation(data: &ZombieData, ice_time: i64, time: i64, animation: Option<&Vec<Num>>) -> PosDistribution {
    ...
    pool.install(|| calculate_animation_impl(data, ice_time, time, animation))
}
...
fn calculate_zomboni(data: &ZombieData, time: i64) -> PosDistribution {
    let mut result = PosDistribution {
        dist: [0.0; 880],
        min: 1000.0,
        max: 0.0
...
#[must_use]
pub fn calculate_pos_distribution(zombie: ZombieType, ice_time: i64, time: i64) -> PosDistribution {
